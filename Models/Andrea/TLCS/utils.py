...
def import_train_configuration(config_file):
    ...
    # Added traffic generator configuration
    config['num_of_seconds'] = content['traffic_generator'].getint('num_of_seconds')
    config['output_file_name'] = content['traffic_generator']['output_file_name']
    return config
...
def set_sumo(gui,sumocfg_file_path ,sumocfg_file_name, max_steps):
    """
    ...
    sumo_cmd = [sumoBinary, "-c", os.path.join(sumocfg_file_path, sumocfg_file_name), "--no-step-log", "true", "--waiting-time-memory", str(max_steps)]
