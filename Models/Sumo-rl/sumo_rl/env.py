...
class SumoEnvironment(gym.Env):
    ...
    def step(self, action: Union[dict, int]):
        ...
        info["rewards"] = rewards['TL101'] if rewards['TL101'] < 0 else 0
    ...
    def _get_system_info(self):
        # OBSERVATIONS ADD NEW HERE HERE
        total_fuel_consumption = 0
        departed_vehicles = traci.simulation.getDepartedIDList()
        number_of_vehciles = len(departed_vehicles)
        ...
        number_of_pedestrians = len(departed_pedestrians)
        ...
        pedestrian_waiting_time = step_pedestrian_queue_length

        return {
            "number_of_vehciles": number_of_vehciles,
            ...
            "accumulated_pedestrian_wait_times": pedestrian_waiting_time,
    ...
    #     queue_length = 0
    #     for lane_id in self.sumo.lane.getIDList():  # Ensure you have all lane IDs in the simulation
    ...
    #         "system_queue_length": queue_length
    #     }
    ...
    def save_csv(self, out_csv_name, episode):
        ...
        """
        # summed_dict = {key: sum(value_list) for key, value_list in self.metrics.items()}
        if out_csv_name is not None:
            df = pd.DataFrame(self.metrics)
            # get the last step number
            step_number=self.metrics[len(self.metrics) - 1]['step']
            ...
            summed_df.to_csv(out_csv_name + f"_conn{self.label}_ep{episode}" + ".csv", index=False)
